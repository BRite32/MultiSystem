...
public class MainActivity extends ActionBarActivity implements
    ...
    private String mDrawerHeaders[] = {"Website", "XDA Thread",
            "Donate via Paypal", "Donate via Gmail", "Become a Fan"};
    private String mAppColor = "#666666";
    private String[] flatColors = {"#666666", "#2980b9", "#16a085", "#e67e22", "#FFC74B46", "#00838F", "#34495e", "#A0522D", "#5D4037", "#7f8c8d"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        File file = new File(getFilesDir()
                + "/.MultiSystem/icon.png");
        if (file.exists()) {
            try {
                getSupportActionBar().setDisplayHomeAsUpEnabled(true);
                getSupportActionBar().setHomeButtonEnabled(true);
                Uri iconUri = Uri
                        .fromFile(new File(getFilesDir() + "/.MultiSystem/icon.png"));
    ...
    public void checkIfConfigExists() {
        ...
        if (!config.exists()) {
            ...
            FileHelper.copyFromAssets(getBaseContext(), configNames,
                    getFilesDir() + "/.MultiSystem");
            FileHelper.copyFromAssets(getBaseContext(), tabNames,
                    getFilesDir()
                            + "/.MultiSystem/tabs");
            FileHelper.copyFromAssets(getBaseContext(), binNames,
                    getFilesDir()
                            + "/.MultiSystem/bin");
        }
    }
        ...
        switch (position) {
            ...
            case 2:
                goToUrl(prefs.getString("TWITTER",
                        "http://forum.xda-developers.com/donatetome.php?u=5868929"));
                break;
            case 3:
                goToUrl(prefs.getString("GOOGLE+",
                        "mailto:hamada.s.badr@gmail.com"));
                break;
            case 4:
                goToUrl(prefs.getString("FACEBOOK",
                        "http://www.facebook.com/hsbadr"));
                break;
            default:
                return;
        }
